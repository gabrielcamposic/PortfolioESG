...
// Lightweight debug logger used throughout this file. Enable by setting window.LEDGER_DEBUG = true in the console.
const DEBUG_MODE = !!window.LEDGER_DEBUG;
function debugLog(...args) {
    if (DEBUG_MODE && console && console.debug) {
        console.debug('[Ledger]', ...args);
    }
}
...
async function renderPipelineSection(ledgerTotalInvested = 0) {
    try {
        ...
        try {
            if (docDir) {
                // e.g. if page is /html/ledger.html, then try /html/../data/... => /data/...
        ...
        try {
            console.debug('[Ledger] pipeline usedPath:', usedPath);
            console.debug('[Ledger] pipeline rows count:', rows ? rows.length : 0);
            if (rows && rows.length > 0) console.debug('[Ledger] pipeline sample last row:', rows[rows.length-1]);
...
// --- INITIALIZATION: ensure mainLedger runs whether the script loaded before or after DOMContentLoaded ---
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() { setTimeout(mainLedger, 0); });
} else {
    // already parsed
    setTimeout(mainLedger, 0);
}
